  ...
  return (
    <div className="relative min-h-screen w-full flex flex-col md:flex-row bg-black text-white font-sans overflow-hidden">
      ...
      <div className="relative flex flex-col justify-center items-center w-full md:w-1/2 min-h-screen p-8 z-20">
        ...
        {/* Decorative Top Glow */}
        <div className="absolute top-0 left-1/4 w-[300px] h-[300px] bg-indigo-500/10 rounded-full blur-[120px] pointer-events-none" />
        ...
        >
          ...
          <div className="mb-6 flex flex-col items-center text-center">
            <a href="/" className="text-3xl font-black tracking-wider bg-gradient-to-r from-indigo-400 via-purple-400 to-pink-400 bg-clip-text text-transparent">
              DevOverflow
            </a>
            <span className="text-xs text-gray-500 font-mono mt-1">
              "Where Developers Overflow with Knowledge"
            </span>
          </div>
      ...
      <div className="relative hidden md:flex flex-col justify-center items-center w-1/2 min-h-screen bg-slate-950 border-l border-white/10 p-12 overflow-hidden">
        ...
        >
          <div className="flex items-center justify-between border-b border-white/10 pb-4 mb-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold font-mono text-gray-300">system@devoverflow:~$</span>
            </div>
          ...
          <div className="space-y-3 font-mono text-xs text-gray-400 select-none">
            {terminalLine >= 0 && (
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-2">
                <span className="text-indigo-400">➜</span> Initializing DevOverflow Environment...
              </motion.p>
            )}
            {terminalLine >= 1 && (
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-2">
                <span className="text-emerald-400 font-bold">[OK]</span> Connection to Appwrite Database Established
              </motion.p>
            )}
            {terminalLine >= 2 && (
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-2">
                <span className="text-emerald-400 font-bold">[OK]</span> Session Authenticator Loaded (JSON Web Token ready)
              </motion.p>
            )}
            {terminalLine >= 3 && (
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-2">
                <span className="text-purple-400">⚡</span> Synchronizing active developer telemetry...
              </motion.p>
            )}
            {terminalLine >= 4 && (
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-gray-300 border border-white/10 bg-white/5 rounded-lg p-3 my-2 leading-relaxed">
                ...
                Active Developers Online: <span className="text-emerald-400">12,492</span><br />
                Reputation Earned Today: <span className="text-purple-400">+49,201 XP</span><br />
                Duplicate Questions Blocked: <span className="text-red-400">381 (AI moderating)</span>
              </motion.p>
            )}
            {terminalLine >= 5 && (
              <motion.p 
              ...
              >
                ...
                <span>Console Listening for user verification credentials...</span>
              </motion.p>
            )}
          </div>
        </motion.div>
        ...
        <div className="mt-8 max-w-lg w-full flex flex-col gap-4 text-sm text-gray-400 select-none">
          <div className="flex gap-3">
            ...
            <p><strong>Ask and Earn:</strong> Gain reputation, rank up, and unlock prestigious animated profile badges.</p>
          </div>
          <div className="flex gap-3">
            <span className="text-purple-400 font-bold">02.</span>
            <p><strong>AI Assistant:</strong> Receive instant code analysis, complexity breakdown, and bug detections right inside answers.</p>
          </div>
        </div>
